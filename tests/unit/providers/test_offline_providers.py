...
from eva.providers.llamacpp_provider import LlamaCppProvider
from eva.providers.llamacpp_provider import get_models as get_llamacpp_models
from eva.providers.ollama_provider import OllamaProvider
from eva.providers.ollama_provider import get_models as get_ollama_models
...
def test_llamacpp_get_models_mocked():
    respx.get("http://localhost:8080/v1/models").respond(json={"data": [{"id": "llama-3-8b-instruct.Q4_K_M.gguf"}]})
...
def test_ollama_provider_error_handling(mock_config):
    ...
    with patch("openai.OpenAI") as mock_openai:
        mock_openai.return_value.chat.completions.create.side_effect = openai.APIConnectionError(request=MagicMock())
