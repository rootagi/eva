...
def test_cli_set_key_getpass_prompt():
    with patch("getpass.getpass", return_value="secretkey"), patch.object(app_module, "set_api_key"):
...
def test_cli_explain_no_arg_no_stdin():
    with patch.object(app_module, "dispatch", return_value=iter(["Repo explanation"])):
...
def test_cli_work_provider_error():
    with patch.object(app_module, "dispatch", return_value=iter(["Error: provider offline"])):
