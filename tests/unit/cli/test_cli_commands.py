...
def test_cli_ask_command(tmp_path):
    ...
    with patch("eva.cli.app.dispatch", return_value=iter(["Answer to query"])):
...
def test_cli_explain_command(tmp_path):
    ...
    with patch("eva.cli.app.dispatch", return_value=iter(["Explanation of script"])):
...
def test_cli_explain_directory_target(tmp_path):
    ...
    with patch("eva.cli.app.dispatch", return_value=iter(["Directory structure explanation"])):
...
def test_cli_analyze_command(tmp_path):
    ...
    with patch("eva.cli.app.dispatch", return_value=iter(["Analysis of error"])):
...
def test_cli_work_dry_run():
    with patch("eva.cli.app.dispatch", return_value=iter(["ls -la"])):
...
def test_cli_work_unsafe_command_refusal():
    with patch("eva.cli.app.dispatch", return_value=iter(["rm -rf /"])):
...
def test_cli_edit_command_apply(tmp_path):
    ...
    with patch("eva.cli.app.dispatch", return_value=iter([diff_output])):
...
def test_cli_commit_command():
    ...
    with (
        patch("eva.cli.app.run_git", return_value=mock_res),
        patch("eva.cli.app.dispatch", return_value=iter(["feat: add change"])),
    ):
...
def test_cli_set_key_command():
    with patch("eva.cli.app.set_api_key"):
...
def test_cli_remove_key_command():
    with patch("eva.cli.app.clear_api_key"):
