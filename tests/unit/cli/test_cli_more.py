...
def test_cli_work_auto_confirm_success():
    with (
        patch.object(app_module, "dispatch", return_value=iter(["echo hello"])),
...
def test_cli_explain_piped_stdin():
    with patch.object(app_module, "dispatch", return_value=iter(["Piped stdin explanation"])):
...
def test_cli_analyze_piped_stdin():
    with patch.object(app_module, "dispatch", return_value=iter(["Piped analyze explanation"])):
...
def test_cli_edit_patch_user_declined(tmp_path):
    ...
    with (
        patch.object(app_module, "dispatch", return_value=iter([diff_text])),
        patch("typer.confirm", return_value=False),
    ):
...
def test_cli_edit_invalid_diff(tmp_path):
    ...
    with patch.object(app_module, "dispatch", return_value=iter(["Not a diff format"])):
        res = runner.invoke(app, ["edit", "change a", "-f", str(target)])
