...
def test_version_flag():
    ...
    assert result.exit_code == 0
    assert "Eva 2.2.0" in result.stdout
...
def test_budget_increment_is_locked(monkeypatch, tmp_path):
    ...
    assert stats.requests_this_minute == 30
...
def test_work_parser_allows_absolute_path_rm():
    parsed = parse_safe_command("/usr/bin/rm -rf /")
    assert parsed.command == "/usr/bin/rm -rf /"
