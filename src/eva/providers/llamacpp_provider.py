...
def get_models() -> list[dict]:
    ...
    try:
        resp = httpx.get(f"{host}/v1/models", timeout=5.0)
        if resp.status_code == 200:
            return resp.json().get("data", [])
    except (httpx.HTTPError, KeyError, ValueError, RuntimeError) as exc:
        logger.warning("llama.cpp model list fetch failed: %s", exc)
    return []


class LlamaCppProvider(OpenAICompatibleProvider):
    name = "llamacpp"
    base_url = os.getenv("LLAMACPP_HOST", "http://localhost:8080") + "/v1"
    max_rpm = 999
    max_rpd = 9999
    ...
    def generate_stream(self, system_prompt: str, user_prompt: str, context: str, config: AppConfig) -> Iterator[str]:
        api_key = get_api_key(self.name) or "llamacpp"
        import openai
        ...
        base_url = os.getenv("LLAMACPP_HOST", "http://localhost:8080").rstrip("/") + "/v1"
        ...
        try:
            ...
            raise ServerError(f"llama.cpp server unavailable at {base_url}: {e}")
        except openai.APIStatusError as e:
            raise ServerError(f"llama.cpp server error: {e}")
